#include "C:\Users\oni\Desktop\mlv3\Aqua rework\src\headers\main.h"
...
ImColor mainColor = ImColor(204.f, 32.f, 41.f, 255.f); // Dark Red for base color
ImColor secondColor = ImColor(255.f, 87.f, 34.f, 255.f); // Bright Orange for hover state
ImColor activeColor = ImColor(255.f, 193.f, 7.f, 255.f); // Bright Yellow for clicked state
...
bool custom::imageButton(ID3D11ShaderResourceView* ptr, float posX, float posY, bool active) {
	...
	ImGui::PushStyleColor(ImGuiCol_Button, conv(mainColor));
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, conv(secondColor));
	ImGui::PushStyleColor(ImGuiCol_ButtonActive, conv(activeColor));
